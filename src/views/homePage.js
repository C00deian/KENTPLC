...
export function renderHomePage(root) {
  root.innerHTML = `
    ...
    <main>
      ...
      <section class="purpose section-pad"><div class="purpose-copy reveal"><span class="eyebrow">${homeContent.purpose.eyebrow}</span><h2>${homeContent.purpose.title}</h2></div><a class="video reveal" href="#" aria-label="Play our purpose film"><img src="${image('videothumb_website-1.jpg', 1800, '16:9')}" alt="Arkmont people" /><span class="play">▶</span><small>Play our film</small></a></section>
      <section class="what section-pad" id="services"><div class="what-grid"><div class="what-image reveal"><img src="${image('arkmont-production-1-1_2025-10-13-172336_pslr.jpg', 1200, '3:4')}" alt="Arkmont engineer" /></div><div class="what-copy reveal"><span class="eyebrow">${homeContent.whatWeDo.eyebrow}</span><h2>${homeContent.whatWeDo.title}</h2>${homeContent.whatWeDo.paragraphs.map(p => `<p>${p}</p>`).join('')}</div></div><div class="service-list">${serviceGroups.slice(0, 5).map((service, i) => `<a href="#" class="service-card reveal"><span>0${i + 1}</span><h3>${service.title}</h3><p>${service.description}</p><b>↗</b></a>`).join('')}</div></section>
      <section class="transition section-pad" id="transition"><div class="transition-copy reveal"><span class="eyebrow yellow">${homeContent.transition.eyebrow}</span><h2>${homeContent.transition.title}</h2>${homeContent.transition.paragraphs.map(p => `<p>${p}</p>`).join('')}<a class="pill light" href="#">Read more about our market capabilities <b>↗</b></a></div><div class="transition-img reveal"><img src="${image('arkmont-production-1-1_2025-10-13-172336_pslr.jpg', 1600)}" alt="Energy transition landscape" /></div></section>
      <section class="reach section-pad" id="projects"><div class="reach-head reveal"><span class="eyebrow">${homeContent.reach.eyebrow}</span><h2>${homeContent.reach.title}</h2><p>${homeContent.reach.intro}</p></div><div class="map reveal"><div class="world"></div>${globalStats.map(stat => `<div class="stat"><strong>${stat.value}</strong><span>${stat.label}</span></div>`).join('')}</div><div class="reach-foot reveal"><p>${homeContent.reach.body}</p><a class="pill dark" href="#contact">Find our office <b>↗</b></a></div></section>
      <section class="careers" id="careers"><div class="career-photo"><img src="${image('arkmont-production-1-1_2025-10-13-172336_pslr.jpg')}" alt="Arkmont colleagues" /></div><div class="career-copy reveal"><span class="eyebrow yellow">${homeContent.careers.eyebrow}</span><h2>${homeContent.careers.title}</h2><p>${homeContent.careers.body}</p><a class="pill light" href="#">Learn more <b>↗</b></a></div><div class="vacancies"><div class="vacancy-heading"><span class="eyebrow yellow">Our Latest Vacancies</span><h2>Find your place at Arkmont</h2></div><form class="job-filters"><label>Keywords<input id="job-keyword" type="search" placeholder="Job title or city" /></label><label>Profession<select id="job-category"><option value="">All professions</option>${[...new Set(vacancies.map(job => job.category))].map(value => `<option>${value}</option>`).join('')}</select></label><label>Location<select id="job-location"><option value="">All locations</option>${[...new Set(vacancies.map(job => job.country))].sort().map(value => `<option>${value}</option>`).join('')}</select></label><button type="submit">Search <b>→</b></button></form><div class="filter-summary"><span aria-live="polite"><b id="job-count">${vacancies.length}</b> opportunities</span><button id="clear-jobs" type="button">Clear filters ×</button></div><div class="jobs" id="jobs-list" aria-live="polite" aria-label="Current vacancies"></div></div></section>
      <section class="latest section-pad" id="news"><div class="latest-head reveal"><div><span class="eyebrow">Arkmont</span><h2>Latest News & Insights</h2></div><div class="arrows"><button class="prev">←</button><button class="next">→</button></div></div><div class="news-track">${newsItems.map(n => `<article class="news-card"><div class="news-img"><img src="${image(n[3], 900)}" alt="" onerror="this.src='${image('videothumb_website-1.jpg', 900)}'"/></div><small>${n[0]}</small><h3>${n[1]}</h3><p>${n[2]}</p><a href="#">Read article <b>↗</b></a></article>`).join('')}</div></section>
    </main>
    <footer id="contact"><div class="footer-top"><img src="/logo-primary.png" alt="arkmont" /><span>Precision in Commissioning.</span></div><div class="footer-grid"><div><small>Office Location</small><p style="color:#c5c3d4; margin-top:10px;">Gomati Nagar Lucknow<br/>Main Office</p></div><div><small>Corporate details</small><p style="color:#c5c3d4; font-size:11px; line-height:1.6; margin-top:10px;">Arkmont Technology Private Limited<br/>CIN: SRNAC3404031<br/>admin@arkmont.in</p></div><a href="#" class="backtop">Back to Top ↑</a></div><div class="legal"><span>© Arkmont Technology Private Limited 2026</span>${footerData.legal.map(link => `<a href="#">${link}</a>`).join('')}</div></footer>
